...
- **Signed updates** — checks GitHub Releases after startup, notifies when a
  newer version exists, and installs verified updates from the Account menu
...
```bash
./Slackinux_0.1.0_amd64.AppImage
```

If FUSE is unavailable, use `--appimage-extract-and-run`. Install the `.deb`
with `sudo apt install ./Slackinux_0.1.0_amd64.deb`.
...
- **Account** — sign in to Slackinux, open Slack Web in your browser, check for
  updates, toggle Do Not Disturb, clear cache & restart
- **View** — zoom and reload controls
- **Window** — minimize, maximize/restore (accelerators work without a visible menubar)
- **Help** — About Slackinux
...
- `settings.json` — persisted zoom level and DND state
...
Slackinux checks the latest GitHub Release shortly after startup. When a newer
signed version is available, it asks before downloading and installing it.
Use **Account → Check for Updates** to check manually at any time.

Updates are verified with the public key embedded in Slackinux. Release builds
and `latest.json` are generated by the GitHub Actions release workflow. The
private signing key is stored only in the repository's encrypted Actions
secret and the project owner's permission-restricted local backup.
